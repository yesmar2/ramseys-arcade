...
export const games: Game[] = [
  ...
  },
  {
    ...
    how: 'Every scene is packed with bugs, and before each one a card shows you who to find — a beetle most often, sometimes a bee, an ant, a grasshopper or a butterfly — by its colours, a hat, a pair of glasses and something held up in one hand, down to the band on the hat. Plenty of the crowd have some of those — only one has all four. Tap the card away when you are ready; the clock waits for you, and the scene stays hidden until you do. Forget who it was? Tap the face by the clock, or press C, and the card comes back while the clock waits again. Tap the bug to move on. Pinch or scroll to zoom and drag to look around; on a keyboard, arrows steer a cursor, space taps and plus and minus zoom. A wrong tap leaves you dazed for a moment, and a scene that runs a full minute shows you where the bug was. Fastest total time wins.',
    accent: PALETTE.teal,
    playable: true,
    inDevelopment: true,
  },
  {
    ...
    how: 'Drag anywhere to fly: the ship moves with your finger, never under it. On a keyboard, arrows or WASD fly and Shift slows you down. Your ship fires on its own, and holding still tightens its shot. Only the white dot at its heart can be hit; everything else can brush straight through a bullet. A bullet that passes close without touching is grazed: it turns gold, warms your multiplier and charges your Barrage. Let the Barrage go with its button, or Space, X or Z, and a wave rolls out that turns every bullet it meets into a star for you, the gold ones worth five times as much, and hammers every ship it passes. Your ship grows stronger as you clear waves, and a lost ship costs a level. Every fifth wave a flagship comes in with phases of its own; bring it down for an extra ship. Three ships to start.',
    accent: PALETTE.red,
    playable: true,
  },
  {
  ...
  },
  {
    ...
    how: 'Five long holes, each somewhere of its own: a garden with a windmill and a creek, a formal garden round a fountain, a castle, a beach out to a lighthouse, and a mountain. Every hole is longer than the screen: the intro flies it from cup to tee, the view follows the ball, and the map in the corner shows the rest, so plan the route. Before a swing, drag or tap the map, scroll, or hold the up and down arrows to look along the hole; the swing brings the view back. It shoots like a slingshot: take hold of the ball and pull it back, and it comes back with your finger on its bands while dots run ahead the way it will go; let go and it snaps forward. The further you pull, the harder it goes, and the last of a full pull sends it a long way. A press anywhere else pulls the same way. Bring it back to where it sits to change your mind. The rails are wherever the green ends. Sand drags, and a shot out of it comes out short; water costs a stroke and sends you back, and so does rolling over an edge; a bridge is the dry way over. Some things keep time: a windmill’s sails shut its doors as they pass, a drawbridge and a portcullis rise and fall, and a sandbar is only dry while the tide is out. A ramp flies the ball further the harder it’s hit: too soft and it drops short, too hard and it flies long, and a crooked one lands off to the side. A drain and a cave take the ball somewhere else. Slopes lean a rolling ball, steps and ridges send a timid one back down, a plaza dishes into its fountain, a wind off the sea pushes toward the water, and the last cup sits on a flat top up on a hill: short and the ball rolls back down, long and it runs off the far side. Crabs scuttle, a beam swings round, boulders and planters stand where they are, and a ball rolling too fast skips over the cup. The score is golf: par pays 200, a birdie 300, a bogey 100, an ace extra. On a keyboard, arrows aim and holding Space charges the shot; let go to hit.',
    accent: PALETTE.green,
    playable: true,
    inDevelopment: true,
  },
  ...
  },
  {
    ...
    how: 'Each firefly glows its own colour and sings its own note. A run is a string of nights, and each night hangs five lanterns across the sky: win a round and one lights, and when all five are lit they ring out and the next night begins. Three rounds a night are tunes: watch the fireflies play one, then tap them in the same order — on a keyboard, each one’s number, 1 to 6. The first tune is three notes and every one after is a note longer. In a Catch, fireflies blink on at random for eight seconds; tap each one while it’s lit, and catch eight to light the lantern, but three taps on a dark one end it. In a Follow, one glows gold, then they all go pale and trade places; tap the one you followed. Each new night brings a new firefly or more trading places. A wrong note in a tune ends the run; a lost Catch or Follow only leaves its lantern dark, and the night plays another round.',
    accent: PALETTE.violet,
    playable: true,
  },
]
