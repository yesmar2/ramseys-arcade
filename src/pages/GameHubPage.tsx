...
import { gameHref, gamePlayHref, leaderboardHref, recordsHref } from '../hooks/useHashRoute'
...
export function GameHubPage({ slug, period: periodFromRoute }: GameHubPageProps) {
  ...
  const otherGames = useMemo(
    () => (boardSlug ? LEADERBOARD_GAMES.filter((s) => s !== boardSlug) : []),
    [boardSlug],
  )

  useEffect(() => {
    setPeriod(periodFromRoute ?? 'daily')
  }, [periodFromRoute, slug])
  ...
  }, [loading, you, boardSlug, period])

  const selectPeriod = (next: LeaderboardPeriod) => {
    window.location.hash = gameHref(slug, next)
  }
          ...
          {boardSlug ? (
            <>
              ...
              <section
                ...
                aria-label={`${game.name} ${PERIOD_LABELS[period]} leaderboard`}
              >
                {loading ? (
                  <BoardSkeleton />
                ) : error ? (
              ...
              {otherGames.length > 0 ? (
                <section className="lb-games" aria-labelledby="game-hub-other-heading">
                  <h2 id="game-hub-other-heading" className="lb-games__title">
                    Other games
                  </h2>
                  <ul className="game-grid game-grid--playable lb-other-grid">
                    {otherGames.map((otherSlug, index) => {
                      const other = getGame(otherSlug)
                      ...
                      return (
                        <GameTile
                          ...
                          game={other}
                          index={index}
                          href={gameHref(otherSlug, period)}
                          showOnAllDevices
                        />
                      )
                    })}
              ...
              <a className="game-lobby__all-boards" href={leaderboardHref()}>
                Global rankings
              </a>
            </>
          ) : null}
